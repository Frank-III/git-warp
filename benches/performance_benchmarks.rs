...
fn bench_cow_vs_traditional_copy(c: &mut Criterion) {
    ...
    for (size_name, file_count) in sizes {
        ...
        group.bench_with_input(
            ...
            |b, repo_path| {
                b.iter(|| {
                    let clone_path = repo_path.parent().unwrap().join(format!("cow_clone_{}", rand::random::<u32>()));
        ...
        group.bench_with_input(
            ...
            |b, repo_path| {
                b.iter(|| {
                    let copy_path = repo_path.parent().unwrap().join(format!("trad_copy_{}", rand::random::<u32>()));
...
fn bench_git_operations(c: &mut Criterion) {
    ...
    group.bench_function("analyze_branches", |b| {
        ...
        for i in 0..5 {
            ...
            if git_repo.create_worktree_and_branch(&branch_name, &worktree_path, None).is_ok() {
...
fn create_test_repo_with_files(file_count: usize) -> tempfile::TempDir {
    ...
    for i in 0..file_count / 10 {
        ...
        for j in 0..5 {
            fs::write(dir_path.join(format!("nested_{}.txt", j)), format!("Nested file {} content", j)).unwrap();
        }
    }
...
fn create_repo_with_config_files(file_count: usize) -> tempfile::TempDir {
  ...
  "path": "{}",
  "build_dir": "{}/build"
}}"#, i, repo_path.display(), repo_path.display());
        
        fs::write(node_modules.join(format!("config_{}.json", i)), config_content).unwrap();
    }
...
fn setup_git_repository() -> tempfile::TempDir {
    ...
    Command::new("git").args(&["config", "user.email", "bench@example.com"]).current_dir(repo_path).output().unwrap();
    Command::new("git").args(&["config", "user.name", "Bench User"]).current_dir(repo_path).output().unwrap();
...
fn copy_directory_recursive<P: AsRef<Path>, Q: AsRef<Path>>(
    src: P, 
    dst: Q
) -> std::io::Result<()> {
    let src = src.as_ref();
