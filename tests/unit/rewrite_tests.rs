...
build_dir = "{}/build"
cache_dir = "{}/cache"
"#, src_dir.display(), src_dir.display(), src_dir.display());
...
"#;
  ...
  "scripts": {{
...
}}"#, src_dir.join("lib/main.js").display(), src_dir.display());
    ...
    let main_config_content = format!(r#"
...
"#, src_dir.display());
    ...
    let build_config_content = format!(r#"
BUILD_ROOT = "{}"
SOURCE_DIR = "{}"
"#, src_dir.display(), src_dir.display());
    ...
    match result {
        Ok(()) => {
            ...
            let package_content = fs::read_to_string(node_modules_dir.join("package.json")).unwrap();
...
fn test_python_virtual_environment_rewriting() {
    ...
    let pyvenv_content = format!(r#"home = /usr/bin
...
"#, src_dir.display(), src_dir.display(), src_dir.display());
    ...
    let activate_content = format!(r#"#!/bin/bash
VIRTUAL_ENV="{}/venv"
export VIRTUAL_ENV
...
}}"#, src_dir.join("bin/tool").display(), src_dir.display(), src_dir.display());
    
    fs::write(package_dir.join("package.json"), package_json).unwrap();
    
    // Create nested dependency
    let nested_dir = node_modules.join("nested").join("node_modules").join("deep-package");
    fs::create_dir_all(&nested_dir).unwrap();
    
    let nested_config = format!(r#"{{
  "rootPath": "{}"
}}"#, src_dir.display());
