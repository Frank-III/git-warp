...
fn test_cow_clone_basic_functionality() {
    ...
    match result {
        Ok(()) => {
            ...
            let nested_content = fs::read_to_string(dest_dir.join("subdir").join("nested.txt")).unwrap();
...
fn test_cow_clone_large_structure() {
    ...
    match result {
        Ok(()) => {
            ...
            assert!(dest_dir.join("node_modules").join("lodash").join("package.json").exists());
            assert!(dest_dir.join("build").join("app.js").exists());
            
            println!("CoW clone of complex structure succeeded: {} files", src_count);
        },
        Err(e) => {
            println!("CoW clone failed (expected on non-APFS): {}", e);
        }
...
fn test_cow_clone_preserves_permissions() {
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = fs::metadata(src_dir.join("script.sh")).unwrap().permissions();
    ...
    match result {
        Ok(()) => {
            ...
            {
                use std::os::unix::fs::PermissionsExt;
                let src_perms = fs::metadata(src_dir.join("script.sh")).unwrap().permissions();
                let dest_perms = fs::metadata(dest_dir.join("script.sh")).unwrap().permissions();
                assert_eq!(src_perms.mode(), dest_perms.mode(), "Permissions not preserved");
            }
