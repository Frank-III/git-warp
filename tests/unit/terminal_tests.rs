use git_warp::terminal::{TerminalManager, Terminal, TerminalMode};
...
fn test_terminal_commands_generation() {
    ...
    #[cfg(target_os = "macos")]
    {
        use git_warp::terminal::{ITerm2, AppleTerminal};
        ...
        if iterm.is_supported() {
            println!("iTerm2 would generate commands for path: {}", path.display());
        }
        
        if apple_terminal.is_supported() {
            println!("Terminal.app would generate commands for path: {}", path.display());
        }
    }
...
fn test_concurrent_terminal_operations() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let paths: Vec<_> = (0..3).map(|i| {
            tempdir().unwrap().path().join(format!("branch-{}", i))
        }).collect();
