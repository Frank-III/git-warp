            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        println!("Git worktree registration failed: {}", 
...
fn test_cow_with_path_rewriting() {
    ...
    match cow_result {
        Ok(()) => {
            ...
            let original_script = fs::read_to_string(clone_path.join("scripts").join("build.sh")).unwrap();
            ...
            match rewrite_result {
                Ok(()) => {
                    ...
                    let rewritten_config = fs::read_to_string(clone_path.join("config.toml")).unwrap();
                    ...
                    let rewritten_script = fs::read_to_string(clone_path.join("scripts").join("build.sh")).unwrap();
    ...
    match cow_result {
        Ok(()) => {
            ...
            if clone_duration.as_secs() < 5 {
                ...
                println!("CoW performance acceptable: {} seconds", clone_duration.as_secs());
            }
        },
        Err(e) => {
            println!("CoW clone failed (expected on non-APFS): {}", e);
...
fn test_cow_filesystem_detection() {
    ...
    match cow_supported {
        Ok(true) => {
            ...
            assert!(dst.exists());
            assert_eq!(fs::read_to_string(dst.join("test.txt")).unwrap(), "test content");
        },
        Ok(false) => {
...
build_dir = "{}/build"
cache_dir = "{}/cache"
"#, repo_path.display(), repo_path.display(), repo_path.display());
...
"#, repo_path.display(), repo_path.display());
    ...
    // Add to git
    Command::new("git").args(&["add", "."]).current_dir(repo_path).output().unwrap();
    Command::new("git").args(&["commit", "-m", "Add configs"]).current_dir(repo_path).output().unwrap();
...
fn setup_git_repository_with_history() -> tempfile::TempDir {
    ...
    for i in 1..=5 {
        fs::write(repo_path.join(format!("file_{}.txt", i)), format!("Content {}", i)).unwrap();
        Command::new("git").args(&["add", "."]).current_dir(repo_path).output().unwrap();
        Command::new("git").args(&["commit", "-m", &format!("Commit {}", i)]).current_dir(repo_path).output().unwrap();
    }
...
fn setup_repository_with_large_deps() -> tempfile::TempDir {
    ...
    for pkg in &["package-1", "package-2", "large-package"] {
        ...
        for i in 0..20 {
            fs::write(pkg_dir.join(format!("file_{}.js", i)), format!("// File {} content", i)).unwrap();
        }
        
        fs::write(pkg_dir.join("package.json"), format!(r#"{{"name": "{}"}}"#, pkg)).unwrap();
    }
    ...
    for i in 0..10 {
        fs::write(build_dir.join(format!("build_{}.js", i)), format!("// Build artifact {}", i)).unwrap();
    }
    ...
    for i in 0..15 {
        fs::write(cache_dir.join(format!("cache_{}.tmp", i)), format!("Cache data {}", i)).unwrap();
    }
