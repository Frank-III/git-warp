...
fn test_complete_worktree_creation_workflow() {
    ...
    // Create some work
    fs::write(worktree_path.join("feature.txt"), "New feature implementation").unwrap();
...
fn test_process_detection_and_cleanup_workflow() {
    ...
    // Create worktree
    git_repo.create_worktree_and_branch(branch_name, &worktree_path, None).unwrap();
...
"#;
    ...
    let processes = process_manager.find_processes_in_directory(&worktree_path).unwrap();
    
    println!("Found {} processes in worktree", processes.len());
    
    // Try cleanup with process detection
    let worktrees = git_repo.list_worktrees().unwrap();
    let feature_worktrees: Vec<_> = worktrees.into_iter()
        .filter(|w| w.branch == branch_name)
        .collect();
    
    if !feature_worktrees.is_empty() {
        let branch_statuses = git_repo.analyze_branches_for_cleanup(&feature_worktrees).unwrap();
...
fn test_branch_analysis_and_cleanup_workflow() {
    ...
    for (branch_name, should_merge) in branches {
        ...
        let worktree_path = repo_path.join("worktrees").join(branch_name.replace('/', "-"));
        git_repo.create_worktree_and_branch(branch_name, &worktree_path, None).unwrap();
        created_worktrees.push((branch_name.to_string(), worktree_path.clone()));
        
        // Make some changes in the worktree
        std::env::set_current_dir(&worktree_path).unwrap();
        fs::write(worktree_path.join("branch_file.txt"), format!("Content for {}", branch_name)).unwrap();
    ...
    let merged_branches: Vec<_> = branch_statuses.iter()
        .filter(|s| s.is_merged)
        .collect();
    
    let unmerged_branches: Vec<_> = branch_statuses.iter()
        .filter(|s| !s.is_merged)
        .collect();
    
    println!("Found {} merged branches, {} unmerged branches", 
             merged_branches.len(), unmerged_branches.len());
...
fn setup_test_repository_with_dependencies() -> tempfile::TempDir {
  ...
  "dependencies": {
    ...
    fs::write(lodash_dir.join("package.json"), r#"{
  "name": "lodash",
  "version": "4.17.21"
}"#).unwrap();
    
    fs::write(lodash_dir.join("index.js"), "module.exports = require('./lodash');").unwrap();
